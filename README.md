...
**1. NVIDIA NIM** — OpenAI-compatible endpoint at `https://integrate.api.nvidia.com/v1/chat/completions`. Images passed as `image_url` content type with a data URI. Free API keys at build.nvidia.com. Models:
...
**2. OpenAI** — `https://api.openai.com/v1/chat/completions`. Standard OpenAI vision API with `image_url` content blocks. Keys at platform.openai.com. Models:
- `gpt-4o-mini` — GPT-4o Mini (fast, cheap)
- `gpt-4o` — GPT-4o (balanced, default)
- `o1` — o1 (best reasoning, slowest)

**3. Anthropic Claude** — `https://api.anthropic.com/v1/messages`. Uses Anthropic's native messages format: image is passed as a `source` block with `type: "base64"` and `media_type: "image/jpeg"`. Requires `x-api-key` and `anthropic-version: 2023-06-01` headers. Keys at console.anthropic.com. Models:
- `claude-haiku-4-5-20251001` — Haiku (fastest)
- `claude-sonnet-5` — Sonnet (balanced, default)
- `claude-opus-5` — Opus (smartest)

**4. Google Gemini** — `https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={apiKey}`. Uses Google's native generateContent format: image is passed as `inline_data` with `mime_type: "image/jpeg"` and a base64 data string inside a `contents.parts` array. This is the only provider using a completely different API shape. Keys at aistudio.google.com. Models:
- `gemini-2.0-flash` — Flash (fastest, default)
- `gemini-1.5-pro` — 1.5 Pro (balanced)
- `gemini-2.5-pro` — 2.5 Pro (smartest)

**5. Groq** — OpenAI-compatible at `https://api.groq.com/openai/v1/chat/completions`. Ultra-fast inference. Images via `image_url`. Keys at console.groq.com. Models:
- `llama-3.2-11b-vision-preview` — Llama 3.2 11B Vision (default)
- `llama-3.2-90b-vision-preview` — Llama 3.2 90B Vision

**6. Together AI** — OpenAI-compatible at `https://api.together.ai/v1/chat/completions`. Images via `image_url`. Keys at api.together.ai. Models:
...
**7. xAI (Grok)** — OpenAI-compatible at `https://api.x.ai/v1/chat/completions`. Images via `image_url`. Keys at console.x.ai. Models:
- `grok-2-vision-1212` — Grok 2 Vision (default)
- `grok-vision-beta` — Grok Vision Beta

**8. Mistral** — OpenAI-compatible at `https://api.mistral.ai/v1/chat/completions`. Pixtral models are native vision models trained by Mistral. Images via `image_url`. Keys at console.mistral.ai. Models:
- `pixtral-12b-2409` — Pixtral 12B (default)
- `pixtral-large-2411` — Pixtral Large

**9. DeepSeek** — OpenAI-compatible at `https://api.deepseek.com/v1/chat/completions`. Images via `image_url`. Keys at platform.deepseek.com. Models:
